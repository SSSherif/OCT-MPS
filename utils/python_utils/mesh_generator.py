...
def create_mesh_file(geo_file: str, output_mesh_file: str):
    ...
    with open(output_mesh_file, 'w+') as output_file:
        ...
        for point in points:
            output_file.write(' '.join([str(p) for p in point.p])+str('\n'))

        for tetrahedron in elements3d:
            output_file.write(' '.join([str(p) for p in list(tetrahedron.vertices)+[tetrahedron.index]])+str('\n'))
